...
import { MoreHorizontal, ThumbsUp, MessageCircle, Share2, Phone, Sparkles } from 'lucide-react';
...
export default function AdPreviewCard({ platform, creative }: AdPreviewCardProps) {
  if (platform === 'google') {
    return (
      <div className="bg-white rounded-3xl border border-slate-200 p-6 space-y-3.5 shadow-xs font-sans text-left">
        <div className="flex items-center space-x-2 text-xs text-slate-500">
          <GoogleAdsLogo className="w-4 h-4" />
          <span className="font-bold text-slate-900 text-[11px] bg-slate-100 px-2 py-0.5 rounded-full">Tajaan</span>
    ...
    return (
      <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-xs font-sans text-left max-w-sm mx-auto">
        ...
        <div className="p-3.5 flex items-center justify-between border-b border-slate-50">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              ...
              <span className="text-[10px] text-slate-400 block">Tajaan · Facebook & Instagram Feed</span>
            </div>
          </div>
        ...
        <div className="p-3 bg-slate-50 flex items-center justify-between border-b border-slate-100">
          ...
          <button
            type="button"
            className="px-3.5 py-1.5 rounded-lg bg-[#0866FF] hover:bg-blue-600 text-white text-xs font-medium shadow-xs transition-colors shrink-0"
          >
        ...
        <div className="px-3.5 py-2 flex items-center justify-between text-xs text-slate-500 bg-white">
          <div className="flex items-center space-x-1">
            <ThumbsUp className="w-3.5 h-3.5 text-[#0866FF]" />
